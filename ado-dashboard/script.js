...
document.addEventListener("DOMContentLoaded", () => {
  ...
  showStatusMessage(
    "Please select App and Sprint, then click Apply to load data",
    "info",
  );
...
function initializeFilters() {
  ...
  sprintSelector = new SprintSelector(
  ...
  applyBtn.addEventListener("click", () => {
    ...
    if (!selectedSprint) {
      ...
      showStatusMessage(
        "Please select a sprint (Project → Team → Sprint)",
        "error",
      );
      return;
    }
...
async function loadAllData() {
  ...
  try {
    ...
    await Promise.all([loadWorkItems(), loadDefects()]);
    ...
    loadTestExecution().catch((err) =>
      console.log(
        "[Load All] Test execution metrics not available:",
        err.message,
      ),
    );
    loadQualityMetrics().catch((err) =>
      console.log("[Load All] Quality metrics not available:", err.message),
    );
...
async function loadWorkItems() {
  ...
  workItemsContent.appendChild(
    EmptyState.createLoading("Loading work items..."),
  );
  ...
  try {
    ...
    if (cachedData) {
      console.log(
        "[Work Items] ✅ Using cached data, items:",
        cachedData.workItemDetails?.length || 0,
      );
    ...
    console.log(
      "[Work Items] Stored in currentData:",
      currentData.workItems.length,
      "items",
    );
    ...
    workItemsContent.innerHTML = "";
    workItemsContent.appendChild(
      EmptyState.createError(error.message, () => loadWorkItems()),
    );
  }
}
...
async function loadDefects() {
  ...
  try {
    ...
    if (cachedData) {
      console.log(
        "[Defects] ✅ Using cached data, defects:",
        cachedData.defects?.length || 0,
      );
    ...
    console.log(
      "[Defects] Stored in currentData:",
      currentData.defects.length,
      "defects",
    );
    ...
    defectsContent.innerHTML = "";
    defectsContent.appendChild(
      EmptyState.createError(error.message, () => loadDefects()),
    );
  }
}
...
async function loadTestExecution() {
  ...
  testExecutionContent.appendChild(
    EmptyState.createLoading("Loading test execution data..."),
  );
  ...
  try {
    ...
    if (cachedData) {
      console.log(
        "[Test Execution] ✅ Using cached data, stories:",
      ...
      testExecutionContent.insertBefore(
        indicator,
        testExecutionContent.firstChild,
      );
      return;
    }
    ...
    const response = await fetch(url);
    console.log(
      "[Test Execution] Response status:",
      response.status,
      response.statusText,
    );
    ...
    testExecutionContent.innerHTML = "";
    testExecutionContent.appendChild(
      EmptyState.createError(error.message, () => loadTestExecution()),
    );
  }
}
...
async function loadQualityMetrics() {
  ...
  const qualityMetricsContent = document.getElementById(
  ...
  qualityMetricsContent.appendChild(
    EmptyState.createLoading("Loading quality metrics..."),
  );
  ...
  try {
    ...
    if (cachedData) {
      console.log(
        "[Quality Metrics] ✅ Using cached data:",
        cachedData.metrics,
      );
      ...
      qualityMetricsContent.insertBefore(
        indicator,
        qualityMetricsContent.firstChild,
      );
      return;
    }
    ...
    const response = await fetch(url);
    console.log(
      "[Quality Metrics] Response status:",
      response.status,
      response.statusText,
    );
    ...
    console.log(
      "[Quality Metrics] Stored in currentData:",
    ...
    qualityMetricsContent.innerHTML = "";
    qualityMetricsContent.appendChild(
      EmptyState.createError(error.message, () => loadQualityMetrics()),
    );
  }
}
...
function updateOverview() {
  ...
  console.log("[Overview] Metric card elements:", {
    activeWorkItems,
    activeDefects,
    testPassRate,
    qualityScore,
...
function updateDefectsByEnvChart() {
  ...
  const envData = currentData.defectMetrics?.byEnvironment || {
    dev: 0,
    uat: 0,
    prod: 0,
  };
...
function updateWorkItemsTab() {
  ...
  const children = currentData.workItems.filter(
    (item) => item.type === "Task" || item.parentId,
  );
...
function createParentCard(parent, children) {
  ...
  const completedCount = children.filter(
    (c) => c.state === "Done" || c.state === "Closed",
  ).length;
  const inProgressCount = children.filter(
    (c) => c.state === "In Progress" || c.state === "Active",
  ).length;
  const todoCount = children.filter(
    (c) => c.state === "To Do" || c.state === "New",
  ).length;
  ...
  const hasDescription =
    parent.description || parent.acceptanceCriteria || parent.reproSteps;
  const descriptionText = stripHtml(
    parent.description || parent.reproSteps || "",
  );
...
async function viewStoryAnalysis(story) {
  ...
  const tabBtns = document.querySelectorAll(".tab-btn");
  tabBtns.forEach((b) => b.classList.remove("active"));
  const analysisTabBtn = document.querySelector(
    '.tab-btn[data-tab="story-analysis"]',
  );
  if (analysisTabBtn) {
...
function updateDefectsTab() {
  ...
  console.log("[Defects Tab] Stat box elements:", {
    defectsDev,
    defectsUat,
    defectsProd,
    defectsCritical,
  ...
  if (defectsCritical) {
    ...
    console.log(
      "[Defects Tab] Set defectsCritical:",
      defectsCritical.textContent,
    );
  }
  ...
  if (!container) {
    console.error(
      "[Defects Tab] defectsList container still not found after recreation attempt!",
    );
    return;
  }

  console.log(
    "[Defects Tab] Populating defects list with",
    currentData.defects.length,
    "defects",
  );
  container.innerHTML = "";
  ...
  currentData.defects.forEach((defect, index) => {
    console.log(
      `[Defects Tab] Adding defect ${index + 1}:`,
      defect.id,
      defect.title,
    );
  ...
  console.log(
    "[Defects Tab] ✅ Defects list populated with",
    currentData.defects.length,
    "defects",
  );
}
...
function updateTestExecutionTab() {
  console.log(
    "[Test Execution Tab] Updating test execution tab (story-centric view)...",
  );
  const testExecutionContent = document.getElementById("testExecutionContent");
  ...
  if (
    !currentData.testExecutionStories ||
    currentData.testExecutionStories.length === 0
  ) {
    // Show empty state
    ...
    if (testExecutionContent) {
      ...
      const cacheIndicator =
        testExecutionContent.querySelector(".cache-indicator");
      if (cacheIndicator) cacheIndicator.remove();
  ...
  console.log(
    "[Test Execution Tab] Creating story-centric test execution view...",
  );
  testExecutionContent.innerHTML = `
        <div class="stats-row">
            <div class="stat-box">
  ...
  document.getElementById("storiesWithTests").textContent =
    metrics.storiesWithTests || 0;
  document.getElementById("totalTestCases").textContent =
    metrics.totalTestCases || 0;
  document.getElementById("overallAutomationRate").textContent =
    `${metrics.overallAutomationRate || 0}%`;
...
function updateTestStoriesList() {
  ...
  if (
    ...
    container.innerHTML =
      '<div style="padding: 20px; text-align: center; color: var(--text-muted);">No stories found for the selected sprint.</div>';
    return;
  }

  console.log(
    "[Test Stories List] Displaying",
    currentData.testExecutionStories.length,
    "stories",
  );

  currentData.testExecutionStories.forEach((story, index) => {
    console.log(
      `[Test Stories List] Adding story ${index + 1}:`,
      story.storyId,
      story.storyTitle,
    );
...
function updateTestRunsList() {
  ...
  console.log(
    "[Test Runs List] Displaying",
    displayRuns.length,
    "test runs (of",
    currentData.testRuns.length,
    "total)",
  );

  displayRuns.forEach((run, index) => {
    console.log(
      `[Test Runs List] Adding test run ${index + 1}:`,
      run.id,
      run.name,
    );
...
function updateQualityMetricsTab() {
  ...
  console.log("[Quality Metrics Tab] Quality metrics data:", {
    ...
    keysCount: currentData.qualityMetrics
      ? Object.keys(currentData.qualityMetrics).length
      : 0,
  ...
  if (
    !currentData.qualityMetrics ||
    Object.keys(currentData.qualityMetrics).length === 0
  ) {
    // Show empty state
    console.log(
      "[Quality Metrics Tab] No quality metrics, showing empty state",
    );
    if (qualityMetricsContent) {
      // Remove any cache indicator if present
      const cacheIndicator =
        qualityMetricsContent.querySelector(".cache-indicator");
      if (cacheIndicator) cacheIndicator.remove();
  ...
  if (!qmDefects && qualityMetricsContent) {
    console.log(
      "[Quality Metrics Tab] Recreating quality metrics tab structure...",
    );
    qualityMetricsContent.innerHTML = `
  ...
  console.log("[Quality Metrics Tab] Metric card elements:", {
    qmDefects,
    qmPassRate,
    qmCoverage,
    qmScore,
  ...
  if (qmPassRate) {
    ...
    console.log(
      "[Quality Metrics Tab] Set qmPassRate:",
      qmPassRate.textContent,
    );
  }
  if (qmCoverage) {
...
function updateQualityDefectsChart() {
  ...
  if (!container) {
    console.error(
      "[Quality Defects Chart] qualityDefectsChart element not found!",
    );
    return;
  }
  ...
  container.innerHTML = `
        <div style="padding: 20px;">
            <div style="margin-bottom: 20px;">
                ...
                ${Object.entries(defects.bySeverity || {})
                  .map(
                    ([key, value]) => `
            ...
            <div>
                ...
                ${Object.entries(defects.byEnvironment || {})
                  .map(
                    ([key, value]) => `
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
...
function updateQualityTestingChart() {
  ...
  if (!container) {
    console.error(
      "[Quality Testing Chart] qualityTestingChart element not found!",
    );
    return;
  }
  ...
  console.log(
    "[Quality Testing Chart] Testing data:",
    testing,
    "Coverage data:",
    coverage,
  );
...
async function analyzeStoryFull(storyId) {
  ...
  try {
    ...
    console.log(
      "[Story Analyzer] Story response status:",
    ...
    try {
      ...
      console.log(
        "[Story Analyzer] Analysis response status:",
      ...
      console.warn(
        "[Story Analyzer] Requirements analysis error (non-fatal):",
        analysisError,
      );
    }
    ...
    if (requirementsAnalysis) {
      ...
      if (requirementsAnalysis.technicalDetails) {
        technicalDetailsFromAPI = requirementsAnalysis.technicalDetails;
      }
    ...
    const displayData = {
      workItem: {
        ...
        technicalDetails: technicalDetailsFromAPI,
...
async function generateTestCases(storyId) {
  ...
  try {
    ...
    console.log(
      "[Test Generator] Response status:",
      response.status,
      response.statusText,
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to generate tests (${response.status}): ${errorText}`,
      );
    }
    ...
    if (error.message.includes("404")) {
      ...
      showStatusMessage(
        "Test case generation endpoint not configured. Please set up /api/ado/generate-test-cases",
        "error",
      );
    } else {
...
function displayAnalysisResults(data) {
  ...
  try {
    ...
    if (data.requirementsAnalysis) {
      ...
      if (req.testCoverageRecommendation) {
        const testTypes = [
          {
            label: "Functional",
            value: req.testCoverageRecommendation.functional,
            color: "#10b981",
          },
          {
            label: "Integration",
            value: req.testCoverageRecommendation.integration,
            color: "#3b82f6",
          },
          {
            label: "Negative",
            value: req.testCoverageRecommendation.negative,
            color: "#f59e0b",
          },
          {
            label: "Edge Case",
            value: req.testCoverageRecommendation.edgeCase,
            color: "#8b5cf6",
          },
          {
            label: "Total",
            value: req.testCoverageRecommendation.total,
            color: "#ec4899",
          },
        ];
    ...
    container.textContent =
      "Error rendering analysis results. Check console for details.";
    return;
  }
...
function displayTestCaseResults(data) {
  ...
  let html =
    '<div style="background: var(--bg-secondary); padding: 20px; border-radius: 12px; border: 2px solid var(--border-color); margin-top: 20px;">';
  html +=
    '<h3 style="color: var(--text-primary); margin-bottom: 16px;">✅ Test Cases Generated</h3>';
  ...
  if (data.testCases && data.testCases.length > 0) {
    data.testCases.forEach((tc, i) => {
      ...
      if (tc.steps && tc.steps.length > 0) {
        html +=
          '<div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color);"><strong>Steps:</strong></div>';
    ...
    html +=
      '<div style="padding: 20px; text-align: center; color: var(--text-muted);">No test cases generated</div>';
  }
...
function initializeTestCasesTab() {
  ...
  console.log("Test cases tab elements:", {
    generateBtn,
    clearBtn,
    storyIdInput,
  });
...
async function generateTestCasesForTab(storyId) {
  ...
  const includeNegative =
    document.getElementById("includeNegativeTests")?.checked || false;
  const includeEdgeCases =
    document.getElementById("includeEdgeCases")?.checked || false;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to generate test cases (${response.status}): ${errorText}`,
      );
    }
...
function displayTestCasesInTab(data) {
  ...
  let html =
    '<div style="background: var(--bg-secondary); padding: 20px; border-radius: 12px; border: 2px solid var(--border-color); margin-top: 20px;">';
  html +=
    '<h3 style="color: var(--text-primary); margin-bottom: 16px;">✅ Generated Test Cases</h3>';
  ...
  if (testCases.length === 0) {
    ...
    testCases.forEach((tc, i) => {
      html +=
        '<div style="background: var(--bg-primary); padding: 16px; border-radius: 8px; margin-bottom: 12px; border-left: 3px solid var(--accent);">';
      ...
      if (tc.preconditions && tc.preconditions.length > 0) {
        html +=
          '<div style="margin-top: 12px;"><strong style="color: var(--text-primary);">Preconditions:</strong></div>';
      ...
      if (tc.postconditions && tc.postconditions.length > 0) {
        html +=
          '<div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color);"><strong>Postconditions:</strong></div>';
        html += '<ul style="margin-left: 20px; margin-top: 4px;">';
      ...
      if (tc.testData && Object.keys(tc.testData).length > 0) {
        html +=
          '<div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color);"><strong>Test Data:</strong></div>';
        html +=
          '<div style="background: var(--bg-secondary); padding: 8px; border-radius: 4px; margin-top: 4px; font-family: monospace; font-size: 12px;">';
        html +=
          '<pre style="margin: 0; white-space: pre-wrap; color: var(--text-secondary);">' +
...
async function loadExistingTestCases(storyId) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/ado/test-cases/by-story/${storyId}`,
    );
