...
export class PushPreviewModal {
  ...
  generateTags() {
    ...
    if (this.analysisData.integration) {
      const integrationCount =
        this.analysisData.integration.summary?.total || 0;
    ...
    if (this.analysisData.blastRadius) {
      const affectedCount =
        this.analysisData.blastRadius.impact?.affectedComponents?.length || 0;
      if (affectedCount > 0) {
  ...
  async fetchPreview(fieldUpdates) {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/ado/update-story/preview`,
        {
          method: "POST",
  ...
  displayFieldsPreview(preview) {
    ...
    if (preview.changes.length === 0) {
      container.innerHTML =
        '<p class="empty-state">No field changes detected</p>';
      return;
    }
  ...
  async executePush() {
    ...
    try {
      ...
      const updateResponse = await fetch(
        `${API_BASE_URL}/api/ado/update-story`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      const commentResponse = await fetch(
        `${API_BASE_URL}/api/ado/add-comment`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
   ...
   */
  getTagClass(tag) {
    if (tag.includes("Risk: CRITICAL") || tag.includes("Risk: HIGH"))
      return "tag-high";
