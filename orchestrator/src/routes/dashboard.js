...
router.get("/javascript-analysis", async (req, res) => {
  try {
    ...
    // Transform to dashboard format
    const dashboardData = transformJavaScriptAnalysisForDashboard(
      response,
      coverage,
      appName,
    );
...
router.get("/overview", async (req, res) => {
  try {
    ...
    const [dotnetCode, dotnetCoverage, jsCode, jsCoverage] = await Promise.all([
      req.mcpManager.callDockerMcp("dotnetCodeAnalyzer", "/analyze", {
        app: appName,
        includeTests: true,
      }),
      req.mcpManager.callDockerMcp("dotnetCoverageAnalyzer", "/analyze", {
        app: appName,
      }),
      req.mcpManager
        .callDockerMcp("javascriptCodeAnalyzer", "/analyze", {
          app: appName,
          includeTests: false,
        })
        .catch(() => ({ analysis: { components: [], functions: [] } })),
      req.mcpManager
        .callDockerMcp("javascriptCoverageAnalyzer", "/analyze", {
          app: appName,
        })
    ...
    const overview = {
      ...
      backend: {
        ...
        untestedMethods: (dotnetCoverage.coverage?.methods || []).filter(
          (m) => !m.hasTests,
        ).length,
      },
      frontend: {
        ...
        untestedFunctions: (jsCoverage.coverage?.functions || []).filter(
          (f) => !f.hasTests,
        ).length,
      },
      combined: {
        totalFiles:
          (dotnetCode.analysis?.totalFiles || 0) +
          (jsCode.analysis?.totalFiles || 0),
        totalCodeUnits:
          (dotnetCode.analysis?.methods?.length || 0) +
          (jsCode.analysis?.functions?.length || 0),
        averageCoverage: Math.round(
          ((dotnetCoverage.coverage?.overallPercentage || 0) +
            (jsCoverage.coverage?.overallPercentage || 0)) /
            2,
        ),
        totalUntested:
          (dotnetCoverage.coverage?.methods || []).filter((m) => !m.hasTests)
            .length +
          (jsCoverage.coverage?.functions || []).filter((f) => !f.hasTests)
            .length,
      },
    };
...
function transformCodeAnalysisForDashboard(codeData, coverageData, appName) {
  ...
  const files = Array.from(fileMap.values()).map((file, idx) => ({
    ...
    avgComplexity:
      file.methods.length > 0
        ? file.methods.reduce((sum, m) => sum + (m.complexity || 0), 0) /
          file.methods.length
        : 0,
    maxComplexity:
      file.methods.length > 0
    ...
    branchCoverage:
      coverageData?.filesCoverage?.[file.name]?.branchCoverage || 0,
    uncoveredLines:
      coverageData?.filesCoverage?.[file.name]?.uncoveredLines || [],
  ...
  return {
    ...
    dependencies: {
      total:
        (analysis.integrations?.epic?.length || 0) +
        (analysis.integrations?.financial?.length || 0),
      circular: 0,
      unused: 0,
    },
    dependencyDetails: [],
 ...
 */
function transformJavaScriptAnalysisForDashboard(
  codeData,
  coverageData,
  appName,
) {
  ...
  const files = Array.from(fileMap.values()).map((file, idx) => ({
    ...
    avgComplexity:
      file.functions.length > 0
        ? file.functions.reduce((sum, f) => sum + (f.complexity || 0), 0) /
...
function detectFileType(filePath) {
  ...
  if (lower.endsWith(".config.js") || lower.endsWith(".config.ts"))
    return "Config";
...
router.get("/aod-summary", async (req, res) => {
  try {
    ...
    logger.info("[Dashboard] Getting ADO summary", {
      sprint,
      state,
      environment,
    });
    ...
    logger.info(
      `[Dashboard] Checking ${workItems.length} work items for parent relations`,
    );

    workItems.forEach((wi) => {
      if (wi.relations) {
        logger.info(
          `[Dashboard] Work item ${wi.id} has ${wi.relations.length} relations`,
        );
    ...
    if (parentIds.size > 0) {
      ...
      const parentWorkItemsResponse = await req.mcpManager
        .callDockerMcp("azureDevOps", "/work-items/query", {
          workItemIds: Array.from(parentIds),
        })
        .catch((err) => {
          logger.warn(
            "[Dashboard] Failed to fetch parent work items:",
            err.message,
          );
          return { data: [] };
        });
      // Unwrap MCP response
      parentWorkItems = parentWorkItemsResponse?.data || [];
    }
    ...
    const workItemDetails = filteredItems.map((wi) => {
      ...
      return {
        ...
        assignedTo:
          wi.fields?.["System.AssignedTo"]?.displayName || "Unassigned",
        priority: wi.fields?.["Microsoft.VSTS.Common.Priority"] || 3,
        tags: wi.fields?.["System.Tags"] || "",
        iterationPath: wi.fields?.["System.IterationPath"] || "",
        areaPath: wi.fields?.["System.AreaPath"] || "",
        createdDate:
          wi.fields?.["System.CreatedDate"] || new Date().toISOString(),
        changedDate:
