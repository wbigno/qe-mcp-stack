...
function resolveFilePath(app, file) {
  ...
  if (!appPath) {
    throw new Error(
      `Unknown app: ${app}. Valid apps: ${Object.keys(appPathMap).join(", ")}`,
    );
  }
...
router.post("/analyze-file", async (req, res) => {
  try {
    ...
    const fileClasses = (analysisData.classes || []).filter(
      (c) => c.file && c.file.includes(fileName),
    );
    const fileMethods = (analysisData.methods || []).filter(
      (m) => m.file && m.file.includes(fileName),
    );

    logger.info(
      `[Test Analysis] Found ${fileClasses.length} classes, ${fileMethods.length} methods in ${fileName}`,
    );
    ...
    const hasIntegrationPoints = fileMethods.some((m) =>
      m.calls?.some(
        (call) =>
          call.includes("Epic") ||
          call.includes("Financial") ||
          call.includes("HttpClient") ||
          call.includes("Repository"),
      ),
    );

    // Get uncovered methods
    const coverageData = coverage.coverage || coverage;
    const untestedMethods = (coverageData.methods || []).filter(
      (m) => m.file && m.file.includes(fileName) && m.coverage === 0,
    );
    const partialMethods = (coverageData.methods || []).filter(
      (m) =>
        m.file &&
    ...
    const result = {
      ...
      analysis: {
        needsUnitTests:
          !isTestFile &&
          (untestedMethods.length > 0 || partialMethods.length > 0),
        ...
        methods: fileMethods.map((m) => ({
          ...
          coverage:
            coverageData.methods?.find((cm) => cm.name === m.name)?.coverage ||
            0,
        })),
        endpoints: hasApiEndpoints ? extractEndpoints(fileClasses) : [],
        integrations: hasIntegrationPoints
          ? extractIntegrations(fileMethods)
          : [],
      },
    };

    logger.info(
      `[Test Analysis] Result: needs unit=${result.analysis.needsUnitTests}, needs integration=${result.analysis.needsIntegrationTests}`,
    );
...
router.post("/generate-for-file", async (req, res) => {
  try {
    ...
    logger.info(
      `[Test Gen] Generating tests for ${className} in ${file || "unknown file"}`,
    );
    ...
    logger.info(
      `[Test Gen] Using test framework: ${testFramework} for app: ${app}`,
    );
    ...
    try {
      ...
      logger.info(
        `[Test Gen] ✅ Successfully read ${resolvedPath} (${sourceCode.length} bytes)`,
      );
    } catch (error) {
...
router.post("/generate-integration-for-file", async (req, res) => {
  try {
    ...
    if (!app || !apiEndpoint) {
      return res
        .status(400)
        .json({ error: "App and apiEndpoint are required" });
    }
    ...
    logger.info(
      `[Test Gen] ✅ Successfully generated integration tests for ${apiEndpoint}`,
    );
...
function extractEndpoints(classes) {
  ...
  classes.forEach((cls) => {
    const routePrefix =
      cls.attributes
        ?.find((a) => a.includes("Route"))
        ?.match(/Route\("([^"]+)"\)/)?.[1] || "/api";

    cls.methods?.forEach((method) => {
      const httpMethod = method.attributes?.find(
        (a) =>
          a.includes("HttpGet") ||
          a.includes("HttpPost") ||
          a.includes("HttpPut") ||
