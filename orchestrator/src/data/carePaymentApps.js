 ...
 * Complete infrastructure data for all 8 CarePayment repositories
...
export const infrastructureData = {
  applications: {
    servicelayer: {
      ...
      features: ['Member Portal Operations', 'Payment Operations', 'PreCare Enrollment', 'Provider Operations', 'Terms & Conditions'],
      integrations: {
        payeezy: {
          name: 'Payeezy',
          type: 'payment',
          ...
          features: ['Card payment processing', 'Transaction authorization', 'Payment capture'],
          endpoints: [
            { method: 'POST', path: '/payment/v1/charges', purpose: 'Process payment' },
            { method: 'POST', path: '/payment/v1/tokens', purpose: 'Tokenize card' }
          ],
          requestStructure: {
            headers: {
          ...
          errorHandling: 'Retry logic with exponential backoff, logs to Application Insights',
...
var result = await response.Content.ReadAsAsync<PayeezyResponse>();`
        ...
        stripe: {
          ...
          type: 'payment',
          color: 'text-green-700',
          purpose: 'Alternative payment processing via Payments API abstraction',
          baseUrl: 'Via Payments API',
          authentication: {
            method: 'Via Payments API',
            config: 'Payments.Api.Url configuration'
          },
          features: ['Card payment processing', 'Tokenization', 'Subscription management'],
          endpoints: [
            { method: 'POST', path: '/Payments/Stripe', purpose: 'Process Stripe payment via Payments API' }
          ],
          requestStructure: {
            body: {
        ...
        paymentsapi: {
          ...
          features: ['Payment processing', 'Card tokenization', 'Payment method storage'],
          endpoints: [
            { 
              method: 'POST', 
...
var response = await client.PostAsJsonAsync(
    ...
    request
        ...
        integrationapi: {
          ...
          features: ['Payment provider integrations', 'Tokenization', 'Digital engagement tracking'],
          endpoints: [
            { method: 'GET', path: '/v1/accounts/accountNumber/{accountNumber}', purpose: 'Get account by account number' },
            { method: 'POST', path: '/v1/payments', purpose: 'Process payment' },
            { method: 'POST', path: '/v1/tokenize', purpose: 'Tokenize payment method' }
          ],
...
var account = await response.Content.ReadAsAsync<AccountResponse>();`
        ...
        niceincontact: {
          ...
          purpose: 'Call center integration for agent management and call logging',
          baseUrl: 'https://api-c48.nice-incontact.com/inContactAPI/services/v22.0/',
          authentication: {
            method: 'OAuth 2.0 Client Credentials',
            config: 'NiceInContact.Api.* settings'
          },
          features: ['Agent lookup', 'Call logging', 'Contact management', 'Skill assignment'],
          endpoints: [
            { method: 'POST', path: '/token', purpose: 'Get OAuth token' },
            { method: 'GET', path: '/agents/{agentId}', purpose: 'Get agent details' },
...
var agentResponse = await client.GetAsync($"/agents/{agentId}");`
        ...
        revspring: {
          ...
          features: ['Payment processing', 'Statement generation', 'Correspondence management'],
          endpoints: [
            { method: 'POST', path: '/payments', purpose: 'Process payment' },
            { method: 'GET', path: '/statements/{accountId}', purpose: 'Get statements' }
          ],
          configuration: {
...
await db.StringSetAsync($"Account:{accountNumber}", 
    ...
    TimeSpan.FromMinutes(15)
        ...
        hangfire: {
          ...
          features: ['Recurring jobs', 'Fire-and-forget jobs', 'Delayed jobs', 'Continuations'],
          jobs: [
            {
...
BackgroundJob.ContinueJobWith(jobId, () => SendConfirmation(paymentId));`,
        ...
        sqlserver: {
          ...
          databases: ['CarePayment', 'MessageContext', 'CBRContext', 'UserAccountContext'],
          tables: {
...
public async Task<CardAccount> GetAccountAsync(string accountNumber) {
    ...
    return account;
          ...
          connectionStrings: {
            primary: 'Data Source=server;Initial Catalog=CarePayment;User Id=xxx;Password=xxx',
        ...
        azure: {
          ...
          services: {
            ...
            eventGrid: {
              ...
              events: ['Payment processed', 'Account updated', 'Statement generated']
            }
          },
...
await client.SendEventAsync(new EventGridEvent("PaymentProcessed", "CarePayment", "1.0", data));`
    ...
    carelink: {
      ...
      features: ['Account Management', 'Settlement Processing', 'Payment Processing', 'Statement Management', 'Compliance Documents'],
      integrations: {
        servicelayer: {
          name: 'ServiceLayer API',
          type: 'api',
          ...
          baseUrl: 'http://localhost:15155/',
          authentication: { method: 'API Key', config: 'ServiceLayerAccessKey' },
          features: ['Account operations', 'Payment processing', 'Transaction history']
        },
        integrationapi: {
          name: 'Integration API',
          type: 'api',
    ...
    memberportal: {
      ...
      features: ['Account Summary', 'Payment Processing', 'Payment Methods', 'Statements', 'Contact Management'],
      integrations: {
        servicelayer: { 
          name: 'ServiceLayer API', 
          type: 'api', 
          ...
          baseUrl: 'http://localhost:15155/',
          authentication: { method: 'API Key', config: 'ServiceLayerAccessKey' },
          features: ['Account data', 'Payment processing', 'Statement retrieval']
        },
        azureadb2c: { 
          name: 'Azure AD B2C', 
          type: 'auth', 
          ...
          authentication: { method: 'OAuth 2.0 / OpenID Connect', config: 'ida:* settings' },
          features: ['Sign-in', 'Sign-up', 'Password reset', 'Profile management']
        },
        paymentsapi: { 
          name: 'Payments API', 
          type: 'api', 
      ...
      dependencies: {
        core: ['CarePayment.ServiceLayer.Api.Models', 'CarePayment.DAL', 'CarePayment.Infrastructure'],
        nuget: ['Core.Common.Shared', 'Entity Framework 6.x', 'Microsoft.IdentityModel.Tokens']
      }
    },
    providerportal: {
      ...
      authentication: 'Forms Authentication with SQL Membership',
      features: ['Account Management', 'Payment Processing', 'Reporting'],
      integrations: {
        servicelayer: {
          name: 'ServiceLayer API',
          type: 'api',
        ...
        azure: {
          ...
          color: 'text-blue-600',
          purpose: 'Cloud services',
          services: { applicationInsights: { name: 'Application Insights', purpose: 'Monitoring' } }
        },
        redis: {
          name: 'Redis Cache',
          type: 'cache',
      ...
      dependencies: {
        core: ['CarePayment.DAL', 'CarePayment.Infrastructure', 'CarePayment.ServiceLocation'],
        nuget: ['Core.Common.Shared', 'Entity Framework 6.x', 'DevExpress v12.1']
      }
    },
    integrationapi: {
      name: 'Integration API',
      ...
      features: ['Payment Processing', 'Tokenization', 'Recurring ACH', 'Digital Engagement', 'Dialer Eligibility'],
      integrations: {
        payeezy: {
          ...
          baseUrl: 'https://cert.api.firstdata.com',
          authentication: { method: 'API Secret', config: 'PayeezyPaymentJsApiSecret' },
        ...
        azure: {
          ...
          purpose: 'Cloud services',
          services: {
            applicationInsights: { name: 'Application Insights', purpose: 'Telemetry' },
            eventGrid: { name: 'Event Grid', purpose: 'Event publishing' },
            tableStorage: { name: 'Table Storage', purpose: 'Notification audit' }
          }
        },
        marketingcloud: {
        ...
        hangfire: {
          ...
          purpose: 'Background processing',
          jobs: [
            { name: 'Recurring ACH Processing', schedule: 'Hourly', description: 'Process recurring ACH payments' },
            { name: 'Digital Engagement Events', schedule: 'Real-time', description: 'Process engagement events' }
          ]
        },
        sqlserver: {
          name: 'SQL Server',
      ...
      dependencies: {
        core: ['CarePayment.Integration.Api.Data', 'CarePayment.Integration.Api.PaymentProviders', 'CarePayment.DAL'],
        nuget: ['Core.Common.Shared', 'Entity Framework 6.x', 'Hangfire', 'FluentValidation']
      }
    },
    corecommon: {
      ...
      baseUrl: 'https://api.carepayment.com',
      authentication: 'JWT Bearer Tokens, API Keys',
      features: ['CarePayment Ops', 'FiServ Ops', 'OnFile Ops', 'Epic Ops', 'OmniChannel Ops', 'PreCare Ops', 'CRM Ops'],
      integrations: {
        azure: {
          ...
          services: {
            applicationInsights: { name: 'Application Insights', purpose: 'Telemetry' },
            keyVault: { name: 'Key Vault', purpose: 'Secret management', uri: 'https://kv-usw1-dev.vault.azure.net/' },
      ...
      dependencies: {
        core: ['CarePayment.Core.Common.Services', 'CarePayment.Core.Common.Data', 'CarePayment.Core.Common.Core'],
        nuget: ['Core.Common.Shared', 'Entity Framework Core', 'Hangfire', 'Serilog', 'SendGrid', 'Azure SDK']
      }
    },
    payments: {
      name: 'Payments API',
      ...
      features: ['Card Payments', 'Tokenization', 'Account Lookup', 'Logo Information'],
      integrations: {
        payeezy: {
          name: 'Payeezy (First Data)',
          type: 'payment',
          ...
          baseUrl: 'https://cert.api.firstdata.com',
          authentication: { method: 'API Secret + HMAC', config: 'Payeezy configuration' },
        ...
        corecommonapi: {
          ...
          features: ['Account information', 'Logo configuration', 'Payment information']
        },
        azure: {
      ...
      dependencies: {
        core: ['CarePayment.Payments.Services', 'CarePayment.Payments.Data', 'CarePayment.Payments.Core'],
        nuget: ['Entity Framework Core', 'Serilog', 'Application Insights', 'Azure SDK', 'AspNetCore.Authentication.ApiKey']
      }
    },
    precare: {
      name: 'PreCare API',
      ...
      features: ['PreCare Enrollment', 'Document Management', 'Member Portal Registration'],
      integrations: {
        servicelayer: {
          name: 'ServiceLayer API',
          type: 'api',
          ...
          authentication: { method: 'API Key', config: 'ServiceLayerAccessKey' },
          features: ['PreCare enrollment operations', 'Tracking logs', 'Member portal registration'],
          endpoints: [
            { method: 'POST', path: '/PreCare/Submit', purpose: 'Submit PreCare enrollment' },
            { method: 'GET', path: '/PreCare/{id}', purpose: 'Get enrollment by ID' },
            { method: 'PUT', path: '/PreCare/UpdatePreCareInfo', purpose: 'Update enrollment' },
            { method: 'POST', path: '/PreCare/Tracking', purpose: 'Add tracking log' }
          ]
        },
        azureblob: {
          name: 'Azure Blob Storage',
          ...
          authentication: { method: 'Connection string', config: 'BlobStorage configuration' },
          features: ['Terms and conditions storage', 'Consent form storage'],
          configuration: {
            'BlobStorage:ContainerName': 'TermsandConditions',
            'Downloads:ConsentToElectronicCommunications': 'Consent document filename',
            'Downloads:MemberPortalTermsAndConditions': 'Terms document filename'
          }
        },
        appinsights: {
          name: 'Application Insights',
          ...
          color: 'text-blue-600',
          purpose: 'Telemetry and monitoring',
          features: ['Request tracking', 'Exception logging', 'Performance monitoring']
        },
        sqlserver: {
