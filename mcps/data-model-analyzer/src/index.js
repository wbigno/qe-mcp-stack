...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: 'data-model-analyzer-mcp',
    timestamp: new Date().toISOString(),
    version: '1.0.0'
  });
});

app.post('/analyze-models', async (req, res) => {
  try {
    ...
    console.error(`[data-model-analyzer] Error:`, error);
    res.status(500).json({ error: error.message });
  }
});

function performAnalysis(files, config) {
  // Extract entities, properties, relationships
  ...
  // Implement specific logic for data-model-analyzer
  for (const file of files) {
...
app.listen(PORT, () => {
  console.log(`data-model-analyzer MCP running on port ${PORT}`);
  console.log('Endpoints:');
  console.log('  POST /analyze-models');
