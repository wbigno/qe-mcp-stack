 ...
 * 
 * Analyzes state machines and workflows in code using static analysis.
 * Pure static analysis - no AI required.
 * 
 * Communication: JSON via stdin/stdout
 */

import { analyzeStateMachine } from './src/analyzer.js';
...
process.stdin.on('end', async () => {
  try {
    ...
    const result = await analyzeStateMachine({
      app,
      sourceCode,
      entityName
    });
