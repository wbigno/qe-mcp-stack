...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: 'quality-metrics-analyzer-mcp',
...
app.post('/analyze-metrics', async (req, res) => {
  try {
    ...
    console.log(`[quality-metrics-analyzer] Analyzing ${appName}...`);

    const appConfig = await analyzer.loadAppConfig(appName);
    const files = await analyzer.scanCSharpFiles(appConfig.path, false);

    console.log(`[quality-metrics-analyzer] Found ${files.length} files`);
    ...
    console.error(`[quality-metrics-analyzer] Error:`, error);
    res.status(500).json({ error: error.message });
  }
});

function performAnalysis(files, config) {
  // Complexity, maintainability, churn, test health
  ...
  // Implement specific logic for quality-metrics-analyzer
  for (const file of files) {
...
app.listen(PORT, () => {
  console.log(`quality-metrics-analyzer MCP running on port ${PORT}`);
  console.log('Endpoints:');
  console.log('  POST /analyze-metrics');
