...
app.post('/map-integrations', async (req, res) => {
  try {
    ...
    console.log(`[integration-mapper] Integration type filter: ${integrationType || 'all'}`);
    console.log(`[integration-mapper] Include diagram: ${includeDiagram || false}`);
    ...
    console.log(`[integration-mapper] Found ${files.length} C# files to analyze`);
    ...
    console.log(`[integration-mapper] Successfully parsed ${parsedFiles.length} files`);

    // Discover integrations using IntegrationDetector
    const discoveryResult = await detector.discoverIntegrations(appConfig, parsedFiles);
    ...
    if (integrationType && integrationType !== 'all') {
      ...
      filteredIntegrations = discoveryResult.integrations.filter(i => i.type === integrationType);
      filteredByType = { [integrationType]: filteredByType[integrationType] || [] };
    }
    ...
    if (includeDiagram) {
      ...
      const diagram = await detector.generateDiagram(filteredIntegrations, appName);
      response.result.diagram = diagram;
    }

    console.log(`[integration-mapper] Analysis complete: ${filteredIntegrations.length} integrations found`);
    console.log(`[integration-mapper] Integration summary:`, response.result.summary.byType);
